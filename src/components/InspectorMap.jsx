...
import { Navigation, Plus, Sparkles, MapPin, Building2, CheckCircle2, ShieldAlert, ExternalLink, Route } from "lucide-react";
...
export default function InspectorMap({ issues, onIssueDetected, onResolveIssue }) {
  ...
  useEffect(() => {
    ...
    issues.forEach((issue) => {
      ...
      const icon = L.divIcon({
        className: "",
        html: `<div class="${pinClass}" style="width: 20px; height: 20px;"></div>`,
        iconSize: [20, 20],
        iconAnchor: [10, 10]
      });
      ...
      const popupHtml = `
        <div style="font-family: var(--font-sans); width: 230px; padding: 4px;">
          <h4 style="margin: 0 0 4px 0; font-size: 14px; font-weight: 700; color: #fff; display: flex; align-items: center; justify-content: space-between;">
            ${issue.category}
            <span style="font-size: 10px; padding: 2px 6px; border-radius: 99px; background: ${
              isResolved ? "rgba(16, 185, 129, 0.2)" : "rgba(244, 63, 94, 0.2)"
          ...
          ${
            !isResolved
              ? `<button id="btn-resolve-${issue.id}" style="width: 100%; background: var(--accent-primary); border: none; color: white; padding: 5px 8px; border-radius: 4px; font-size: 11px; cursor: pointer; font-weight: 600;">Mark Resolved by Ward Engineer</button>`
              : `<div style="text-align: center; color: var(--color-success); font-size: 11px; font-weight: 700;">✓ Defect Repaired</div>`
          }
        </div>
      `;
  ...
  useEffect(() => {
    ...
    // Build OSRM URL format: lng,lat;lng,lat
    ...
    <div className="map-view-container animate-fade-in">
      ...
      <div style={{ position: 'relative', height: '100%' }}>
        ...
        {showAddForm && clickCoords && (
          <div style={{
            ...
            <h3 style={{ fontSize: '1rem', fontWeight: 'bold', marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.35rem', fontFamily: 'var(--font-display)' }}>
      ...
      <div className="glass-card flex flex-col" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', overflowY: 'auto' }}>
        ...
        <div>
          ...
          <div style={{ background: 'rgba(0,0,0,0.3)', padding: '0.65rem', borderRadius: 'var(--border-radius-sm)', border: '1px solid var(--border-color)', fontSize: '0.8rem' }}>
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', marginTop: '0.35rem', paddingTop: '0.35rem' }}>
              <strong>Est Drive Time:</strong> {routeStats.durationMins} mins {isLoadingRoute ? "(Updating road geometry...)" : ""}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div 
            ...
            style={{
              ...
              border: '1px solid ' + (selectedRoute === 'direct' ? 'var(--color-danger)' : 'rgba(255,255,255,0.05)'),
              background: selectedRoute === 'direct' ? 'rgba(244, 63, 94, 0.1)' : 'rgba(255,255,255,0.01)',
              cursor: 'pointer'
            }}
          >
          ...
          <div 
            ...
            style={{
              padding: '0.85rem',
              borderRadius: 'var(--border-radius-sm)',
              border: '1px solid ' + (selectedRoute === 'alternate' ? 'var(--color-success)' : 'rgba(255,255,255,0.05)'),
              background: selectedRoute === 'alternate' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(255,255,255,0.01)',
              cursor: 'pointer'
            }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }}>
